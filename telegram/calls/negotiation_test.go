...
// A sequência que uma chamada real produz: nossa oferta é respondida e só
// então o par faz a oferta DELE, com os SSRCs reais. Essa oferta tardia tem
// de ser processada e respondida — descartá-la deixa o par sem resposta, e
// ele nunca começa a mandar mídia. Era a causa do áudio de entrada mudo.
func TestNegotiatorPeerOfferAfterOwnRound(t *testing.T) {
	...
	if reply == nil {
		t.Fatal("a oferta do par precisa ser respondida")
	}
	if !ready {
		t.Fatal("pronto assim que o SSRC do par é conhecido")
	}
...
func TestNegotiatorRemoteOffer(t *testing.T) {
	...
	if got := reply.Contents[0].Ssrc; got != "5000" {
		t.Fatalf("answer audio ssrc = %q, want 5000 (a oferta é ecoada de volta)", got)
	}
