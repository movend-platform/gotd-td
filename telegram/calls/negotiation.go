...
type contentNegotiation struct {
	...
	offered         bool
	answered        bool
	// peerExchangeID é a última oferta do par que respondemos. O par
	// retransmite a mesma oferta até ser respondida, então responder por
	// exchange (e não por mensagem) mantém o handshake idempotente.
...
func (n *contentNegotiation) applyRemoteChannels(msg *negotiateChannelsMessage, audioSSRC, videoSSRC uint32) (reply *negotiateChannelsMessage, ready bool) {
	// Resposta à NOSSA oferta: ela ecoa os NOSSOS conteúdos, então não diz
	// nada sobre os SSRCs do par. Ler isso como "do par" vincula o receptor
	// ao nosso próprio SSRC e descarta toda a mídia de entrada.
	if n.offered && msg.ExchangeID == n.localExchangeID {
		n.answered = true
		return nil, n.peerAudio != 0
	}

	// Retransmissão de uma oferta que já respondemos.
	if msg.ExchangeID == n.peerExchangeID {
		return nil, n.peerAudio != 0
	}

	// A oferta do PAR: é aqui que vivem os SSRCs dele, e ela precisa de uma
	// resposta ecoando os conteúdos ofertados — sem isso o cliente oficial
	// não começa a mandar mídia.
