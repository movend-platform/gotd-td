...
// remoteTrack é um receptor criado antes de a aplicação registrar OnTrack.
...
// OnTrack registra o callback de cada trilha remota. Trilhas criadas ANTES
// do registro são repassadas a fn.
//
// O replay não é luxo: a aplicação só recebe o Conn depois de Accept/Request
// retornar, e a essa altura o InitialSetup do par já pode ter sido
// processado — o que é a regra ao ATENDER, já que quem liga sinaliza no
// instante do aceite. Sem isso os receptores nascem sem destino e quem
// atendeu ouve silêncio a chamada inteira.
...
func (c *Conn) handleNegotiate(data []byte) error {
	...
	// Toda NegotiateChannels precisa ser processada, não só a primeira: as
	// duas direções são negociadas de forma independente, então a oferta do
	// par (a que carrega os SSRCs dele) costuma chegar depois de já estarmos
	// "negotiated" pela nossa rodada. Descartá-la deixava o par sem resposta
	// e o SSRC de áudio dele desconhecido — uma chamada sem mídia de entrada.
	c.mu.Lock()
...
func (c *Conn) maybeCreateChannels() {
	...
	// Emissores são criados uma vez; receptores seguem os SSRCs do par, que
	// só ficam conhecidos quando ele oferta e podem mudar se ele re-ofertar.
...
// deliverTrack entrega a trilha à aplicação, ou a segura até OnTrack existir.
// Ler o callback sob mutex também elimina a corrida entre a goroutine de
// sinalização e a da aplicação.
...
// SetVideoState declara se estamos enviando vídeo, e avisa o par.
//
// Quem escreve em VideoTrack precisa anunciar; até lá o par é informado de
// que não há vídeo — que é o que uma chamada de áudio realmente é.
...
// sendMediaState reporta nosso estado de mídia ao par.
//
// videoState precisa refletir a realidade. Era fixo em "active", o que dizia
// a todo par que havia vídeo mesmo numa chamada só de áudio — a trilha é
// criada sempre, mas nada é escrito nela a menos que o chamador peça. O que o
// cliente do outro lado faz com um vídeo declarado-e-ausente é problema dele,
// e nenhuma das possibilidades é boa.
